...
현재는 로컬 검증이 먼저이므로 인증 없이 실행하지만, 배포 단계에서는 아래 방향으로 접근 제어를 추가할 계획이다.

- 공개 회원가입은 두지 않는다.
- 운영자 계정은 초기 seed 또는 직접 주입으로 생성한다.
- `GET /health`를 제외한 운영 API는 로그인 뒤에만 접근 가능하게 둔다.
- 배포 시 비밀값은 우선 `ADMIN_PASSWORD`, `SESSION_SECRET` 환경변수로 주입하는 방향을 기준으로 둔다.
- 초기 세션은 로그인 시점부터 최대 `4시간` 동안만 유효한 고정 만료를 우선 기준으로 둔다.
- 인증 쿠키는 영구 저장하지 않는 세션 쿠키를 우선 기준으로 둔다.
...
1차 인증 엔드포인트 이름은 아래 정도를 우선 기준으로 본다.
...
배포 단계에서는 위 API 중 `GET /health`만 공개 후보로 두고, 나머지는 로그인 필요 API로 묶는 방향을 기준으로 본다.
