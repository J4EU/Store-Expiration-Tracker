...
이 문서는 `docs/product/mvp.md`를 바탕으로, 실제 구현에 바로 옮길 수 있는 운영 규칙을 정리한다.
...
4. `폐기 완료`와 `폐기 없음`은 모두 현재 소비기한을 방치하지 않지만, 다음 상태 입력 방식은 다를 수 있다.
5. `폐기 완료`는 `expiration_date = NULL`로 바로 전환하고, `폐기 없음`은 새 소비기한 입력 또는 `expiration_date = NULL` 적용으로 이어질 수 있다.
...
- `expiration_states.expiration_date IS NOT NULL`

이 섹션에는 아래 상품이 모두 포함된다.

- 기본 진입인 `오늘 처리`에서는 지난 상품과 오늘 만료 상품을 함께 보여준다.
- `오늘 처리`에는 `유제품`이면서 내일 만료인 상품도 함께 포함한다.
- `전체` 필터에서는 이후 상품까지 포함한 모든 소비기한 등록 상품을 보여준다.
...
2. 따라서 기본 진입에서는 지난 상품과 오늘 만료 상품을 먼저 보여주고, `유제품`처럼 하루 먼저 처리하는 규칙도 포함할 수 있어야 한다.
3. 필요할 때 `전체` 필터에서 이후 상품까지 함께 조회할 수 있어야 한다.
4. `expiration_date`가 `NULL`인 상품은 날짜 비교가 아니라 같은 화면 안의 별도 섹션으로 관리한다.

## 4. 구현 시 확인할 사항

1. `폐기 완료`와 `폐기 없음`은 공통 카드 안에 두되, 후속 상태 입력 방식은 동일하게 강제하지 않는 편이 낫다.
2. `폐기 완료`와 `폐기 없음`은 저장하는 이력의 의미를 분리해야 하며, `폐기 없음`에서는 다음 소비기한 즉시 입력을 우선 유도하는 편이 낫다.
3. `폐기 없음`에서 입력을 강제하지 않아도 되는 이유는 상품이 삭제되지 않고 `expiration_date = NULL`도 `미확인` 상태로 추적되기 때문이다.
4. 메인 대시보드에서 처리 대상과 미확인 대상을 분리해 보여주더라도, 상태 저장 규칙은 단순하게 유지한다.
5. 메인 화면 날짜 입력은 캘린더보다 숫자 직접 입력 중심 UX를 우선 검토한다.
6. 상품 등록 중복 시에는 실패 처리만 하지 말고 기존 상품 정보를 바로 안내할 수 있어야 한다.
7. 분석 기능은 MVP 범위 밖이지만, 폐기 이력은 이후 집계를 고려해 누적 저장한다.
