...
1. 초기 세션 최대 유지 시간은 `4시간`으로 둔다.
2. 이 시간 기준은 실제 운영 사용 시간인 `18:00-22:30` 근무 흐름을 기준으로 잡는다.
3. 브라우저가 종료되면 세션도 함께 종료되는 방향을 우선으로 본다.
4. 무활동 시간 기준 자동 로그아웃은 1차에서는 넣지 않는다.
5. 기기별 세션 정책 분리는 보류하고, 모든 기기에 같은 정책을 먼저 적용한다.
