...
- 배포 단계에서는 공개 회원가입 없이, 미리 생성한 운영자 계정으로만 로그인하는 접근 제어를 목표로 한다
...
배포 전 하드닝 방향은 아래 기준으로 정리하고 있습니다.
...
- 초기 세션 정책은 로그인 시점부터 최대 `4시간` 동안만 유효한 고정 만료를 우선 기준으로 둔다.
...
- 인증 엔드포인트는 `login / logout / session 확인` 정도의 최소 범위로 먼저 둔다.
